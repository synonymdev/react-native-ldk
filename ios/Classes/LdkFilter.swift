...
class LdkFilter: Filter {
    override func register_tx(txid: [UInt8]?, script_pubkey: [UInt8]) {
        ...
        LdkEventEmitter.shared.send(withEvent: .register_tx, body: ["txid": txIdHex, "script_pubkey": scriptPubkeyHex])
    }
    ...
    override func register_output(output: WatchedOutput) -> Option_C2Tuple_usizeTransactionZZ {
        ...
        LdkEventEmitter.shared.send(withEvent: .register_output, body: ["block_hash": blockHashBytes, "index": String(outpoint.get_index()), "script_pubkey": scriptPubkeyHex])
